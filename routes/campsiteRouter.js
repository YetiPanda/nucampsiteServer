...
campsiteRouter
  ...
  .post(authenticate.verifyUser, authenticate.verifyAdmin, (req, res, next) => {
    // If comments are provided, ensure author fields are properly handled
    if (req.body.comments && Array.isArray(req.body.comments)) {
      req.body.comments = req.body.comments.map((comment) => {
        // If author is a string, remove it or set it to the current user
        if (typeof comment.author === "string") {
          comment.author = req.user._id; // Use current user as author
        }
        return comment;
      });
    }
